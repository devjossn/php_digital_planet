<?php
  ...
  class Db extends PDO
  {
          ...
          if (!empty($sSql) && count($aBindWhereParam) <= 0) {
              ...
              try {
                  if ($this->_oSTH->execute()) {
					  ...
					  $this->_errorLog('pdoQuery [pdo.class.php, ln.:' . __line__ . ']', $this->interpolateQuery());
                  }
              }
              catch (PDOException $e) {
                  $error = true;
      ...
      public function select($sTable = '', $aColumn = array(), $aWhere = array(), $sOther = '')
      {
          ...
          Debug::AddMessage("queries", ++self::$count . '. select | <i>total: ' . (($this->affected) . ' ' . ($error ? '<b>(error)</b>' : null)) . '</i>', $this->interpolateQuery(), "session");         return $this;
      }
       ...
       * @return
       */
      public function getValue($sTable, $aColumn, $field = '', $value = '')
      {
          if($row = $this->select($sTable, array($aColumn), array($field => $value), 'LIMIT 1')->result()) {
			  return $row->$aColumn;
		  } else {
			  return false;
		  }
