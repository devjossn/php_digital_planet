  ...
  class Cron
  {
      ...
      public static function runStripe($days)
      {
          ...
          if ($data) {
              try {
                  foreach ($data as $row) {
					  $tax = Content::calculateTax($row->uid);
                      ...
                      $data = array(
                          ...
                          'user_id' => $row->uid,
                          'amount' => $row->amount,
                          'total' => Validator::sanitize($row->amount * $tax, "float"),
					  ...
					  $udata = array(
						  ...
						  'user_id' => $row->uid,
						  'membership_id' => $row->membership_id,
						  'expire' => Date::calculateDays($row->membership_id),
						  'recurring' => 1,
						  'active' => 1,
						  );
					  ...
					  //update user record
					  Db::run()->update(Users::mTable, array('mem_expire' => $udata['expire'], 'membership_id' => $row->membership_id), array("id" => $row->uid));
