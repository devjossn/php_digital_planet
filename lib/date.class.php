<?php
   ...
   * @copyright 2016
   * @version $Id: date.class.php, v1.00 2016-10-20 18:20:24 gewa Exp $
   */
  ...
  class Date
  {
      ...
      public static function calculateDays($membership_id)
      {
          ...
          $row = Db::run()->first(Content::mxTable, array('days', 'period'), array('id' => $membership_id));
          if ($row) {
              switch ($row->period) {
                  case "D":
                      $diff = ' day';
      ...
      public static function monthList($list = true, $long = true, $selected = false)
      {
          ...
          $selected = is_null(Validator::get('month')) ? $fmt->format(new DateTime()) : Validator::get('month');
          $html = '';
          if ($list) {
              foreach ($data as $key => $val) {
                  $html .= "<option value=\"$key\"";
