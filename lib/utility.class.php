<?php
  ...
  class Utility
  {
      ...
      public static function getLogo()
      {
		  $core = App::Core();
          if ($core->logo) {
              $logo = '<img src="' . UPLOADURL . '/' . $core->logo . '" alt="' . $core->company . '" style="border:0;height:60px;padding:16px"/>';
          } else {
              $logo = $core->company;
          }
       ...
       * @return
       */
      public static function salePercent($old, $new)
      {
       ...
       * @return
       */
      public static function renderPrice($sale, $price, $sprice, $color = '')
      {

		  return ($sale) ? 
		  Utility::formatMoney($sprice) . ' <small class="wojo strike ' . $color . ' text margin left">' . Utility::formatMoney($price) . '</small>' :
       ...
       * @return
       */
      public static function toTextArea($array, $key)
      {

		  $temp = '';
		  if($array) {
			  foreach ($array as $row)
				  $temp .= $row->{$key} . PHP_EOL;
		  }
